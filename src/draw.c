...
draw_tilesList(void)
{
  ...
  while (draw_tilesSubList() != 0xFE) {  /* draw sub-list */
    t += (SYSVID_WIDTH<<3);  /* go down one tile i.e. 8 lines */
    fb = t;
  }
}
...
#ifdef GFXPC
    draw_setfb(xs, ys + (r << 3) );
#endif
#ifdef GFXST
    draw_setfb(xs, 8 + ys + (r << 3));
#endif
...
/*  for (k = 0; k < 31*SYSVID_WIDTH; k++)
    fb[k] = 0;
  for (; k < SYSVID_WIDTH * i->h; k++)
    fb[k] = i->pixels[k-(31*SYSVID_WIDTH)];

  */
	/*
for (k = 0; k < 31*SYSVID_WIDTH; k++)
    fb[k] = 0;		 */
  for (k = 0; k < SYSVID_WIDTH * i->h; k++)
    fb[k] = i->pixels[k];


  //slTVOn();
  //slBMPut(0, 0, SYSVID_WIDTH-1, SYSVID_HEIGHT-1, i->pixels);
  //slSynch();
